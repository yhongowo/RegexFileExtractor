...
## 2026-09-26：后台预览、异步保存与复制进度

- 超过 1,000 条结果时，输出计划由一个后台任务计算；连续选择/布局修改只保留最新待处理快照，并取消过期计算。UI 立即更新勾选与统计，清除旧输出路径，计划完成前禁用提取。少量结果保持同步更新。选择缓冲和已移交的输出索引继续复用。
- 配置保存按顺序在后台执行，相邻普通设置保存合并为最新快照，规则切片独立复制。保存失败会提示，当前编辑保留在内存中；退出时再次保存最新状态，成功后才关闭，失败可选择放弃保存。底层临时文件、Sync 和原子替换逻辑不变。手动配置恢复流程仍为原有同步流程。
- 复制新增预检阶段和按成功写入字节节流的反馈（约 100 ms）。进度条表示已处理工作量，包含跳过/失败条目的大小；实际写入量单独记录为 BytesCopied，避免将跳过内容误算为实际传输。状态栏保留跳过与失败数量。预检期间使用活动指示器；单个大文件复制不必等文件结束才更新。
- 计算提示放在状态栏，避免每次勾选都改变设置区高度或触发整块设置面板刷新。

Apple M4 Pro、darwin/arm64、Go 1.27.1，Fyne 测试驱动，`-benchtime=500ms -count=2`：

| 场景 | 本次耗时 |
| --- | ---: |
| 一万条结果，完整计划更新（包含等待后台结果） | 2.09–2.10 ms |
| 一万条结果，连续选择的 UI 请求回调 | 64–66 µs |
| 十万条结果，连续选择的 UI 请求回调 | 134–135 µs |
| 空列表缩放布局 | 35–36 µs |
| 一万条结果缩放布局 | 约 42 µs |

连续选择基准刻意暂停 UI 完成回调以模拟输入合并，不能将其解释为十万条计划计算只需 0.13 ms，也不能将请求次数换算为 FPS。完整计划更新基准仍包含计算与 UI 应用结果；一万条每次约分配 2.08 MiB，优化主要减少 UI 阻塞和重复请求，不声称消除了计划计算成本。测试驱动不测量 Windows GPU 绘制。

新增回归覆盖：旧计划不覆盖新选择、布局切换与失效处理、关闭后丢弃计算结果、保存合并与快照隔离、保存回调顺序、退出刷新和保存失败、异步扫描/复制衔接、预检取消、字节进度及跳过计数、文件中途取消后的目标保护与临时文件清理。
...
go test -tags ci,migrated_fynedo,no_emoji ./internal/ui -run '^$' \
  -bench 'BenchmarkPreviewSelection|BenchmarkWindow.*Resize' -benchtime=500ms -count=2 -benchmem
```
