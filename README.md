...
面向 Windows 10 / 11（amd64）的桌面文件提取工具，使用 Go + Fyne 构建。你可以保存多条正则规则，选择其中一条递归筛选文件名，预览提取结果后将文件复制到目标目录。

![Regex File Extractor 界面示例](docs/example-en.png)

## 使用方法

运行 `dist/RegexFileExtractor.exe`。只需此单个文件，不需要安装 Go、Fyne、.NET 或额外的编译器运行库。Windows 界面使用随程序嵌入的约 1.8 MiB 精简字体，覆盖 GB2312、Latin 和全部界面文案；生僻字符由系统字体回退。Fyne 界面使用系统 OpenGL，需要正常工作的显卡驱动。

1. 点击扫描设置中的文件夹图标，使用 Windows 系统目录选择窗口选择源目录和目标目录。目标目录留空时，程序默认使用 `<源目录>/extracted`；也可以手动输入或选择其他目录。复制时会自动创建目标目录，扫描时会排除该目录。
2. 选择一条规则，或添加、编辑自己的规则。规则编辑器支持输入示例文件名即时测试。
3. 选择 `auto`、`group` 或 `flat` 布局，并决定遇到同名目标文件时跳过还是覆盖。
4. 点击 **扫描文件**。程序始终递归扫描，无需设置递归选项。
5. 查看文件名、完整源路径、大小、命中规则和**目标相对路径**，勾选需要的文件；点击表头复选框可全选或清空选择。点击文字单元格可以查看完整字段并复制详情。
...
点击右上角地球图标即可切换中英文（任务执行期间设置会锁定）。程序会记住源目录、目标目录、所选规则、布局和冲突策略。修改源目录、目标目录或规则后，需要重新扫描，以免复制过期结果。
...
截图由 Fyne 软件测试画布生成，不代表 Windows 实机渲染效果。默认会导出 800×600 窗口，以及 600、640、1200 像素宽度的窄屏布局。设置区保持左右并排；高度不足时可以滚动，结果区会随窗口大小调整。
...
Windows 界面使用 `assets/RFESans-Regular.otf`，字体随 exe 嵌入并在整个进程中复用；常规和粗体样式共用同一资源。生僻字、额外文字及 Emoji 使用系统字体回退。其他平台的开发预览使用 Fyne 默认字体。

## 1.1.0 性能优化

- 升级到 Fyne 2.8.1，发布时启用 `migrated_fynedo`；后台 UI 更新均通过 `fyne.Do` / `fyne.DoAndWait`。标志随 exe 生效，不依赖旁置配置文件。
- Windows 界面使用精简的 CJK 字体，常规和粗体样式共用一份约 1.8 MiB 的资源；其他平台使用 Fyne 默认字体。
- 结果区改为虚拟列表，仅复用可见行；列宽随窗口改变，未变文字不重复刷新。
- 结果行、表头、正则预览和状态栏复用完整文字纹理，通过裁剪和独立省略号跟随缩放，减少重新排版及截断纹理缓存的临时峰值。
- 设置区保持左右并排和自然高度，纵向缩放空间分配给结果区；缩放时不递归刷新设置控件。
...
精简字体资源可用 `fonttools==4.60.2` 重新生成：

```sh
python scripts/subset-font.py /path/to/NotoSansCJKsc-Regular.otf
```

这是开发期工具；当前 Windows 构建会嵌入生成的字体，但构建和运行程序不需要 Python。
