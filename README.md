...
1. 点击扫描设置中的文件夹图标，使用 Windows 系统目录选择窗口选择源目录和目标目录。目标目录可留空，此时输入框以灰色提示显示 `<源目录>/extracted`，并随源目录变化；手动输入或选择目标目录后使用自定义路径，清空后恢复默认。复制时自动创建目标目录，扫描时排除该目录。
...
点击右上角地球图标可直接切换中英文（任务执行期间设置锁定）。源目录、目标目录、所选规则、布局及冲突策略会记住。修改源目录、目标目录或规则后，需要重新扫描，避免复制旧的结果。
...
截图由 Fyne 软件测试画布生成，用于布局检查，非 Windows 实机截图。默认 800×600 窗口及 600、640、1200 像素宽度均会导出。设置区始终左右并排；扫描设置中的操作按钮只显示图标，高度不足时可滚动，结果区随窗口大小自动调整。
